...
//MatchedUsers:
export const matchedUsers = createAsyncThunk('auth/matchedUsers',
    async ({ keyword, sort, subCaste, education, raasi, star, gothra, motherTongue, marritalStatus }, { rejectWithValue }) => {
        let link = '/getMatchUsers';
        ...
        if (sort !== null && sort !== undefined) {
            link += `${keyword ? '&' : '?'}sort=${sort}`
        }

        if (subCaste !== null && subCaste !== undefined && subCaste !== '') {
            link += `${keyword || sort ? '&' : '?'}subCaste=${subCaste}`
        }

        if (education !== null && education !== undefined && education !== '') {
            link += `${keyword || sort || subCaste ? '&' : '?'}education=${education}`
        }
        if (raasi !== null && raasi !== undefined && raasi !== '') {
            link += `${keyword || sort || subCaste || education ? '&' : '?'}raasi=${raasi}`
        }
        if (star !== null && star !== undefined && star !== '') {
            link += `${keyword || sort || subCaste || education || raasi ? '&' : '?'}star=${star}`
        }
        if (gothra !== null && gothra !== undefined && gothra !== '') {
            link += `${keyword || sort || subCaste || education || raasi || star ? '&' : '?'}gothra=${gothra}`
        }
        if (motherTongue !== null && motherTongue !== undefined && motherTongue !== '') {
            link += `${keyword || sort || subCaste || education || raasi || star || gothra ? '&' : '?'}motherTongue=${motherTongue}`
        }
        if (marritalStatus !== null && marritalStatus !== undefined && marritalStatus !== '') {
            link += `${keyword || sort || subCaste || education || raasi || star || gothra || motherTongue ? '&' : '?'}maritalStatus=${marritalStatus}`
        }
...
//get Admin users:
export const getUsersByAdmin = createAsyncThunk('auth/getUsersByAdmin',
...
//get Admin users:
export const removeUserByAdmin = createAsyncThunk('auth/removeUserByAdmin',
    async (id, { rejectWithValue }) => {
        try {
        ...
        builder
            ...
            .addCase(getUsersByAdmin.fulfilled, (state, action) => {
                ...
                if (action.payload.gender === 'male') {
                    state.maleUsersByAdmin = action.payload.users;
                } else if (action.payload.gender === 'female') {
                    state.femaleUsersByAdmin = action.payload.users;
                } else {
                    state.allUsersByAdmin = action.payload.users;
                }
            })
            .addCase(getUsersByAdmin.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.payload || 'Users Not Get';
            })
            ...
            //matchedUsers
            .addCase(matchedUsers.pending, (state) => {
                state.status = 'loading';
                state.error = null;
            })
