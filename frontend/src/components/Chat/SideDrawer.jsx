...
import { getMatchedUsers, loginAuthStatus, matchedUsers } from '../../slices/authSlice';
...
const SideDrawer = () => {
    ...
    useEffect(() => {
        if(search || isOpen){
        dispatch(matchedUsers({ keyword: search }))
    }
    ...
    return (
        <>
            ...
            <button
                className="bg-gray-100 mx-4 border bg-primary text-white flex items-center gap-2  px-3 py-2 rounded-lg z-50"
                ...
                <div className="p-4">
                    ...
                    <div className='mt-2' style={{ overflowX: 'auto', height: 'calc(100vh - 150px)' }}>
                        {Status === "loading" ? (
                            ...
                            MatchedUsers.users?.length > 0 && MatchedUsers.users.map((result, index) => (
                                <div key={index} onClick={() => handleAccessChat(result._id)} className="cursor-pointer flex items-center space-x-4 p-2  rounded-lg  hover:bg-gray-300 bg-gray-100 transition-colors duration-200 ease-in-out">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-gray-500 text-sm">{CalculateAge(result.DOB)} years old</span>
                                    </div>
                                </div>
                            ))
                        )}
